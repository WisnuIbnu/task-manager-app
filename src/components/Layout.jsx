...
const Layout = (onLogout, user) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      if (!token) throw new Error("No auth token found")
        ...
        console.error(error);
        setError(error.massage || "Could not load task")
  ...
  const StatCard = ({title,value, icon}) => (
    <div className='p-2 sm:p-3 rounded-full bg-white shadow-sm border border-purple-100 hover:shadow-md transition-all duration-300 hover:border-purple-100 group'>
      <div className='flex items-center gap-2'>
        ...
        <div className="text-xs to-gray-500 font-medium">
          {title}
        </div>
      </div>
    </div>
          ...
          <div className="xl:col-span-1 space-y-4 sm:space-y-6">
            <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100">
              <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 to-gray-800 flex items-center gap-2">
                  <TrendingUp className='w-4 h-4 sm:w-5 to-purple-500' />
                  Task Statistics
              </h3>

              <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:mb-6">
                <StatCard 
                  title='Total Tasks' 
              ...
              <hr className='my-3 sm:my-4 border-purple-400' />

              <div className="space-y-2 sm:space-y-3">
                <div className='flex items-center justify-between to-gray-700'>
            ...
            <div className="bg-white rounded-xl sm:p-5 shadow-sm border border-purple-100">
              <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 to-gray-800 flex items-center gap-2">
                <Clock className='w-4 h-4 sm:w-5 sm:h-5 to-purple-500'/>
                Recent Activity
              </h3>
                  ...
                  {tasks.slice(0,3).map((task) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium to-gray-700 break-words whitespace-normal">
                          {task.title}
                        </p>

                        <p className="text-xs to-gray-500 mt-0.5">
                          {task.CreatedAt ? new DataTransfer(task.CreatedAt).toLocaleDateString(0)
                          : "No date"}
                        </p>
                        
                        <span className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2 ${task.completed ? 'bg-green-100 to-green-700' : 'bg-fuchsia-100 text-fuchsia-700'}`}>
                  ...
                  {tasks.length === 0 && (
                    <div className='text-center py-4 sm:py-6 px-2'>
                      <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto sm:mb-4 rounded-full bg-purple-100 flex items-center justify-center">
                          <Clock className='w-6 h-6 sm:w-8 sm:h-8 to-purple-500' />
                      </div>
                      <p className="text-sm to-gray-500">
                        No Recent Activity
                      </p>
                      <p className='text-xs to-gray-400 mt-1'>
                        Task will apper hear
                      </p>
                    </div>
                  )}
              </div>
